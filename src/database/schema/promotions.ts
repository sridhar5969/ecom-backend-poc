...
export const promoRules = pgTable('promo_rules', {
	...
	triggerVariantId: uuid('trigger_variant_id').references(
		() => productVariants.id,
	),
	triggerCategoryId: uuid('trigger_category_id').references(
		() => categories.id,
	),
	...
	benefitVariantId: uuid('benefit_variant_id').references(
		() => productVariants.id,
	),
	benefitCategoryId: uuid('benefit_category_id').references(
		() => categories.id,
	),
...
export const loyaltyBalancesRelations = relations(
	loyaltyBalances,
	({ one }) => ({
		user: one(users, {
...
export const loyaltyRedemptionRulesRelations = relations(
